...
class CommentController {
  ...
  async readCommentList(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      const { commentList, lastPageId, count } = await service.readCommentList(+postId, +pageId);
      res.status(200).json({ commentList, lastPageId, count });
    } catch (err) {
      next(err);
    }
  }
  ...
  async readLastCommentList(req: Request, res: Response, next: NextFunction) {
    const postId = req.query.postId as string;
    try {
      const { commentList, lastPageId, count } = await service.readLastCommentList(+postId);
      res.status(200).json({ commentList, lastPageId, count });
    } catch (err) {
      next(err);
    }
