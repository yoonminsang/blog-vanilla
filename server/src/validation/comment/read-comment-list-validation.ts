...
const CHECK_PAGE_ID = 'pageId를 확인해주세요';
...
const readCommentListValidation = (req: Request, res: Response, next: NextFunction): void => {
  try {
    const schema = Joi.object({
      ...
      pageId: Joi.number().integer().positive().required().empty('').messages({
        'number.integer': CHECK_PAGE_ID,
        'number.positive': CHECK_PAGE_ID,
        'any.required': CHECK_PAGE_ID,
      }),
    });
